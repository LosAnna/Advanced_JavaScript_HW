...
function removeReviewFromLS(productName, id) {
  ...
  const targetProductIndex = productsArrayWithReviews.findIndex(
    (product) => product.product === product.Name
  );
  const targetReviewId = productsArrayWithReviews[
    targetProductIndex
  ].reviews.findIndex((review) => review.id === id);
  productsArrayWithReviews[targetProductIndex].reviews.splice(
    targetReviewId,
    1
  );
